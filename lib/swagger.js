...
// Set custom responses for errors or add new custom error responses.
Swagger.prototype.setCustomErrorResponse = function(errorType, responseObj, code) {
  customErrors[errorType] = responseObj;
  Swagger.prototype.errors[errorType] = function (res) {
    if (!res) {
      return responseObj;
    } else {
...
Swagger.prototype.canAccessResource = function(req, path, method, spec) {
  for (var i = 0; i < this.validators.length; i++) {
    var validator = this.validators[i];
    if (_.isFunction(validator) && !validator.apply(null, arguments)) {
      return false;
    }
  }
  return true;
...
Swagger.prototype.addMethod = function(app, callback, spec) {
  ...
  if (allowedMethods.indexOf(currentMethod) > -1) {
    app[currentMethod](fullPath, function (req, res, next) {
      ...
      if (!self.canAccessResource(req, path, req.method, spec)) {
        res.send(JSON.stringify(customErrors.cannotAccess || {
          'code': 403,
          'message': 'forbidden'
        }), 403);
      } else {
